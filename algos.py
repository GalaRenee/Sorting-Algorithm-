...
def bubble_sort_gen(data: List[int]) -> Generator:
    """Bubble sort that yields each step"""
    a = data[:]
    steps = comps = swaps = 0 
...
def quick_select_sort_gen(data: List[int]) -> Generator:
    ...
    # For educational purposes, we'll implement a variant that sorts the entire array
    # but demonstrates the quick select principle
    a = data[:]
    steps = comps = swaps = 0
    start_time = time.time()
...
def get_generator(name: str, data: List[int]):
    ...
    elif "Quick Sort" in name:  # Full name to avoid Quick Select
